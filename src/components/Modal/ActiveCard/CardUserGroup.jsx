...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  // Lấy active board từ redux mới mục đích lấy được danh sách thành viên của board thông qua field FE_allUsers
  const board = useSelector(selectCurrentActiveBoard)
  /**
   * Thành viên trong card sẽ phải là tập con của thành viên trong board
   * Vì thế dựa vào mảng board.FE_allUsers và card.memberIds chúng ta tạo ra một mảng FE_cardMemberIds chứa đủ thông tin của user để hiển thị ra ngoài giao diện, bởi mặc định trong card chỉ lưu mảng id của user
   */
  const FE_cardMemberIds = board?.FE_allUsers?.filter(user =>
  ...
  return (
    <Box sx={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
      ...
      {FE_cardMemberIds.map((user, index) => (
